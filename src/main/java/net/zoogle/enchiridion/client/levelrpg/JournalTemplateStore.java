...
final class JournalTemplateStore {
    ...
    static JournalTemplateStore load() {
    ...
    NormalizedSlotRegion regionFor(JournalPagePurpose purpose, JournalPageSlot slot, NormalizedSlotRegion defaults) {
        return overrides.getOrDefault(purpose, Map.of()).getOrDefault(slot, defaults).clamped();
    }

    void update(JournalPagePurpose purpose, JournalPageSlot slot, NormalizedSlotRegion region) {
        overrides.computeIfAbsent(purpose, ignored -> new EnumMap<>(JournalPageSlot.class)).put(slot, region.clamped());
    }

    JournalTemplateStore copy() {
    ...
    void restoreFrom(JournalTemplateStore other) {
        overrides.clear();
    ...
    record NormalizedSlotRegion(double x, double y, double width, double height) {
